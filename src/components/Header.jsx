  ...
  return (
    <header className={`${isLightMode ? 'bg-white text-black' : 'bg-[#09090b]/95 text-white'} h-16 flex items-center px-4 backdrop-blur-md sticky top-0 z-50 transition-colors`}>
      <div className="max-w-7xl mx-auto w-full grid grid-cols-3 items-center">
        ...
        {/* TITLE SECTION - PERFECTLY CENTERED COLUMN */}
        ...
        {/* TOOLS & PROFILE SECTION - RIGHT COLUMN */}
        <div className="flex items-center justify-end gap-3 justify-self-end">
          ...
          <div className="relative w-full max-w-[180px] hidden md:block">
            ...
            <input 
              ...
              className={`w-full ${isLightMode ? 'bg-black/5 border-black/10 text-black' : 'bg-white/5 border-white/10 text-white'} border rounded-full py-1.5 pl-9 pr-8 text-[10px] outline-none focus:border-[var(--theme)]/50 transition-colors`} 
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="absolute right-2 top-1/2 -translate-y-1/2 p-0.5 hover:bg-white/10 rounded-full text-[var(--theme)]">
                <X className="w-3 h-3" />
              </button>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="relative flex items-center">
              <select 
                ...
                className={`text-[9px] font-bold uppercase py-2 pl-2 pr-6 rounded-lg border transition-all outline-none cursor-pointer appearance-none ${
                  isLightMode 
          ...
          <div className={`flex items-center gap-1 ${isLightMode ? 'bg-black/5 border-black/5' : 'bg-white/5 border-white/5'} rounded-full p-1 border`}>
              <button 
                ...
                className="w-7 h-7 rounded-full border border-transparent hover:border-[var(--theme)] overflow-hidden bg-zinc-800 transition-all active:scale-90"
              >
                {profilePic ? (
