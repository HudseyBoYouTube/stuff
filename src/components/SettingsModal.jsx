...
export function SettingsModal({
  ...
  // Helper for dynamic backgrounds in settings sections
  const sectionBg = isLightMode ? "bg-black/5 border-black/10" : "bg-white/5 border-white/5";
  const inputBg = isLightMode ? "bg-white border-black/10" : "bg-zinc-800 border-white/10";

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className={`${isLightMode ? 'bg-zinc-100 border-black/10' : 'bg-zinc-900 border-white/10'} p-6 rounded-3xl max-w-md w-full relative shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto custom-scrollbar`}>
        ...
        <div className="space-y-6">
          ...
          <section className={`space-y-4 bg-[var(--theme)]/5 p-4 rounded-2xl border ${isLightMode ? 'border-[var(--theme)]/20' : 'border-[var(--theme)]/10'}`}>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                <label className={`p-3 ${inputBg} rounded-xl text-[9px] font-black uppercase text-center cursor-pointer hover:border-[var(--theme)]/50 transition-all`}>
              ...
              <input 
                ...
                className={`w-full ${inputBg} rounded-xl p-3 text-xs outline-none focus:border-[var(--theme)]/50 font-bold`}
              />
              <div className={`${isLightMode ? 'bg-black/5' : 'bg-black/20'} p-3 rounded-xl border ${isLightMode ? 'border-black/5' : 'border-white/5'} space-y-3`}>
                <div className="flex items-center justify-between">
                  <p className="text-[8px] font-black text-zinc-500 uppercase leading-none">Your Friend Code</p>
                  <button 
                    onClick={handleCopyCode}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${copied ? 'bg-green-500 text-black' : isLightMode ? 'bg-black/5 text-zinc-600 hover:bg-black/10 hover:text-black' : 'bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white'}`}
                  >
                    {copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                    {copied ? 'Copied' : 'Copy'}
                  </button>
                </div>
                <div className={`${isLightMode ? 'bg-white/50' : 'bg-white/5'} p-2 rounded-lg border border-black/5 max-h-20 overflow-y-auto no-scrollbar`}>
          ...
          <section className={`space-y-4 ${sectionBg} p-4 rounded-2xl border`}>
            ...
            <div className="flex gap-2">
              <input 
                ...
                className={`flex-1 ${inputBg} rounded-xl p-2.5 text-xs outline-none focus:border-[var(--theme)]/50`}
              />
              <button 
            ...
            <div className="space-y-2 max-h-32 overflow-y-auto custom-scrollbar pr-1">
              {friends?.length > 0 ? friends.map(friend => (
                <div key={friend.code} className={`flex items-center justify-between ${isLightMode ? 'bg-white/60' : 'bg-white/5'} p-2 rounded-xl border border-black/5`}>
            ...
            {bgMusic && (
              <div className={`pt-2 border-t ${isLightMode ? 'border-black/5' : 'border-white/5'} space-y-3`}>
                ...
                <input 
                  ...
                  className="w-full h-1.5 bg-zinc-700/20 rounded-lg appearance-none cursor-pointer accent-[var(--theme)]"
                />
              </div>
            )}
          </section>
          ...
          <section className="space-y-3">
            ...
            {/* LIGHT MODE TOGGLE (FIXED VISIBILITY) */}
            <button 
              onClick={() => setIsLightMode(!isLightMode)}
              className={`w-full p-3 mb-2 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 transition-all border 
                ${isLightMode 
                  ? 'bg-black/5 border-black/10 text-black hover:bg-black/10' 
                  : 'bg-white/5 border-white/10 text-white hover:border-[var(--theme)] hover:bg-white/10'}`}
            >
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(themes || {}).map(([id, t]) => (
                <button 
                  ...
                  className={`p-3 rounded-xl text-[10px] font-bold flex items-center gap-2 transition-all border 
                    ${isLightMode 
                      ? 'bg-black/5 border-black/10 hover:border-[var(--theme)]' 
