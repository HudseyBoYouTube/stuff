...
export function FriendViewModal({ friend, gamesData, onClose, ownPfp, isOwnProfile }) {
  ...
  // This is the raw code string used in the screenshot
  const friendCodeString = isOwnProfile ? friend?.code : friend?.decoded?.code;

  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
      <div className="bg-zinc-900 border border-[var(--theme)]/30 p-8 rounded-3xl max-w-sm w-full relative shadow-[0_0_50px_rgba(0,0,0,0.5)] space-y-6 flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto pr-2 custom-scrollbar space-y-6">
          <div className="text-center space-y-2">
            <div className="w-24 h-24 bg-[var(--theme)]/10 rounded-full mx-auto flex items-center justify-center border border-[var(--theme)]/20 overflow-hidden">
              {displayPfp ? (
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-2">
              {displayFavs.length > 0 ? displayFavs.map(gameId => {
          ...
          {/* THE FIXED SECTION FROM YOUR SCREENSHOT */}
          {isOwnProfile && (
            <div className="space-y-2 pt-4 border-t border-white/5">
              <div className="flex justify-between items-center">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Your Friend Code</label>
              </div>
              <div className="bg-black/50 border border-white/10 rounded-xl p-3 max-h-32 overflow-y-auto custom-scrollbar">
                <p className="text-[10px] font-mono text-blue-400 break-all whitespace-pre-wrap leading-relaxed">
                  {friend?.code || "Loading code..."}
                </p>
              </div>
            </div>
          )}
