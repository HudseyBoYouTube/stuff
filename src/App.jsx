      ...
      <header className={`sticky top-0 z-50 border-b h-16 flex items-center px-4 backdrop-blur-md transition-all ${isDarkMode ? 'bg-[#09090b]/95 border-white/5' : 'bg-white/95 border-zinc-100'}`}>
        <div className="max-w-7xl mx-auto w-full grid grid-cols-3 items-center">
          ...
          <div className="flex items-center gap-2 w-full max-w-sm mx-auto">
            ...
            <button onClick={launchRandom} className={`p-2 border rounded-full hover:bg-[var(--theme)] hover:text-black transition-all ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-zinc-200 shadow-sm'}`}><Dices className="w-5 h-5" /></button>
          </div>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={`border p-6 rounded-3xl max-w-md w-full relative z-10 shadow-2xl transition-all ${isDarkMode ? 'bg-zinc-900 border-white/10' : 'bg-white border-zinc-200'}`}>
            <X onClick={() => setShowSettings(false)} className="absolute top-4 right-4 cursor-pointer text-zinc-400" />
               ...
               <div className={`p-4 rounded-2xl border transition-colors ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-zinc-50 border-zinc-100'}`}>
                  ...
                  <div className="flex gap-3">
                    {['#10A5F5', '#10f57b', '#f5107b', '#f5a610'].map(c => (
                      <button key={c} onClick={() => {setTheme(c); localStorage.setItem('capy-theme', c);}} className={`w-8 h-8 rounded-full border-2 ${theme === c ? 'border-zinc-400 scale-110' : 'border-transparent'}`} style={{background: c}} />
                    ))}
                  </div>
               </div>

               <button onClick={() => { if(confirmClear) { localStorage.clear(); window.location.reload(); } else setConfirmClear(true); }} className="w-full p-4 rounded-2xl border border-red-500/20 text-red-500 text-xs font-bold">
                 <Trash2 className="w-4 h-4 inline mr-2" /> {confirmClear ? "Are you sure?" : "Clear App Data"}
               </button>
            </div>
          </div>
        </div>
