...
import { Search, Gamepad2, Play, Settings, X, ShieldAlert, Keyboard, Power, Heart } from 'lucide-react';
...
function App() {
  ...
  const toggleFavorite = (e, id) => {
    e.stopPropagation(); // Prevents the game from opening when clicking the heart
    setFavorites(prev => 
  ...
  // Separate games into Favorites and Others
    ...
    >
      <div className={`aspect-[4/3] overflow-hidden bg-zinc-800/50 flex items-center justify-center ${
        ...
        <button 
          ...
          className="absolute top-3 right-3 z-10 p-2 rounded-full bg-black/40 backdrop-blur-md border border-white/10 hover:bg-black/60 transition-colors"
        >
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-black border border-white/10 p-6 rounded-2xl max-w-sm w-full relative shadow-2xl">
              ...
              <div className="space-y-6">
                ...
                <select onChange={handlePresetChange} className="w-full bg-black text-zinc-100 border border-white/10 rounded-xl p-3 text-sm appearance-none cursor-pointer">
