...
function App() {
  ...
  if (dataError || gamesData.length === 0) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-6 text-center ${isDarkMode ? 'bg-[#09090b] text-white' : 'bg-zinc-50 text-black'}`}>
  ...
  return (
    <div className={`min-h-screen pb-20 antialiased transition-colors duration-300 ${isDarkMode ? 'bg-[#09090b] text-zinc-100' : 'bg-zinc-50 text-zinc-900'}`} style={{ '--theme': theme }}>
      
      {/* HEADER */}
      <header className={`sticky top-0 z-50 border-b h-16 flex items-center px-4 backdrop-blur-md transition-colors ${isDarkMode ? 'bg-[#09090b]/95 border-white/5' : 'bg-white/80 border-zinc-200'}`}>
          ...
          <div className="flex items-center gap-2 w-full max-w-sm mx-auto">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className={`w-full border rounded-full py-2 pl-10 pr-10 text-xs outline-none focus:border-[var(--theme)]/50 text-center transition-all ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-zinc-100 border-zinc-200 focus:bg-white'}`} 
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-red-500 transition-colors">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
            <button onClick={launchRandom} className={`p-2 border rounded-full hover:bg-[var(--theme)] hover:text-black transition-all shrink-0 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-zinc-200 shadow-sm'}`}><Dices className="w-5 h-5" /></button>
          </div>

          <div className="flex items-center justify-end gap-2">
             <button onClick={() => setShowSettings(true)} className={`p-2 transition-colors ${isDarkMode ? 'text-zinc-500 hover:text-[var(--theme)]' : 'text-zinc-400 hover:text-black'}`}><Settings className="w-6 h-6" /></button>
          </div>
        </div>
      </header>

      {/* COMPACT CATEGORY NAV */}
      <div className={`sticky top-16 z-40 border-b px-4 transition-colors ${isDarkMode ? 'bg-[#09090b]/90 border-white/5' : 'bg-white/90 border-zinc-200'}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 mt-4">
        ...
        {filteredGames.length === 0 && (
          <div className="text-center py-20">
            <p className={`font-black uppercase tracking-tighter text-4xl ${isDarkMode ? 'text-zinc-800' : 'text-zinc-200'}`}>Nothing found</p>
          </div>
        )}
      </main>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className={`border p-6 rounded-3xl max-w-md w-full relative z-10 shadow-2xl transition-colors ${isDarkMode ? 'bg-zinc-900 border-white/10' : 'bg-white border-zinc-200'}`}>
            <X onClick={() => setShowSettings(false)} className="absolute top-4 right-4 cursor-pointer text-zinc-500 hover:text-red-500" />
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 scrollbar-hide">
               ...
               {/* Appearance Switch */}
               <div className={`p-4 rounded-2xl border flex items-center justify-between transition-colors ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-zinc-100 border-zinc-200'}`}>
                  <div className="text-sm flex items-center gap-2">
                    {isDarkMode ? <Moon className="w-4 h-4 text-[var(--theme)]" /> : <Sun className="w-4 h-4 text-orange-500" />} 
                    Dark Mode
                  </div>
                  <button onClick={() => setIsDarkMode(!isDarkMode)} className={`w-10 h-5 rounded-full relative transition-colors ${isDarkMode ? 'bg-[var(--theme)]' : 'bg-zinc-300'}`}>
               ...
               <div className={`p-4 rounded-2xl border flex items-center justify-between transition-colors ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-zinc-100 border-zinc-200'}`}>
                  ...
                  <button onClick={() => setStealthMode(!stealthMode)} className={`w-10 h-5 rounded-full relative transition-colors ${stealthMode ? 'bg-[var(--theme)]' : 'bg-zinc-300'}`}>
                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${stealthMode ? 'left-6' : 'left-1'}`} />
                  </button>
               </div>

               {/* Color Theme Picker */}
               <div className={`p-4 rounded-2xl border transition-colors ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-zinc-100 border-zinc-200'}`}>
                  ...
                  <div className="flex gap-3">
                    {['#10A5F5', '#10f57b', '#f5107b', '#f5a610'].map(c => (
                      <button 
                        ...
                        className={`w-8 h-8 rounded-full border-2 transition-all ${theme === c ? 'border-white scale-110 shadow-lg' : 'border-transparent opacity-60 hover:opacity-100'}`} 
                        style={{background: c}} 
                      />
                    ))}
                  </div>
    ...
    <div 
      className={`group rounded-[2rem] overflow-hidden border transition-all flex flex-col cursor-pointer 
        ${isDarkMode 
          ...
          : 'bg-white border-zinc-200 hover:border-[var(--theme)]/50 hover:shadow-xl hover:shadow-[var(--theme)]/5'}`} 
